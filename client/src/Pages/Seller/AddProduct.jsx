...
import Categories, { categoriesData } from "../../Components/Categeries";
...
const {data} = await axios.post('http://localhost:4000/api/product/add', formData, { withCredentials: true })
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className="max-w-24 cursor-pointer" src={files[index] ? URL.createObjectURL(files[index]): "https://raw.githubusercontent.com/prebuiltui/prebuiltui/main/assets/e-commerce/uploadArea.png"} width={100} height={100} />
                            </label>
                        ))}
                    </div>
                </div>
